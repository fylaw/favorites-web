...
public class FavoritesController extends BaseController {
	...
	public Response addFavorites(String name, Long parentId) {
		...
		if (null != parentId) {
			...
			t.setId(parentId);
			Optional<Favorites> parent = favoritesRepository.findOne(Example.of(t));
			if (!parent.isPresent()) {
		...
		try {
			...
			if (null != fav && getUserId() == fav.getUserId().longValue()) {
				...
				if (null != config) {
					...
					if (null != favorites) {
						configRespository.updateFavoritesById(config.getId(), String.valueOf(favorites.getId()),
